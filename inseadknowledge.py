...
def main():
    ...
    try:
        ...
        logging.info(f"Fetched {len(initial_cards)} articles from initial load.")
        ...
        script_tags = soup.find_all("script", type="application/json", string=re.compile(r"Drupal\.settings"))
        for script_tag in script_tags:
            try:
                ...
                logging.debug("Script tag content was not valid JSON for Drupal.settings.")
                continue
            except Exception as e:
                logging.debug(f"Error parsing script tag for Drupal.settings: {e}")
                continue
        ...
        if drupal_settings_json:
            ...
            views_ajax_settings = drupal_settings_json.get('views', {}).get('ajax', {})
            for view_id, view_data in views_ajax_settings.items():
                if 'dom_id' in view_data:
                    view_dom_id = view_data['dom_id']
                    logging.info(f"Extracted dynamic view_dom_id from JSON: {view_dom_id}")
                    break
            if view_dom_id == "2bcf87ffae10d903e48004546039697aebd0e6dc08d71cbbb4b8e009c1559405":
                logging.warning("Dynamic view_dom_id not found in Drupal.settings JSON. Using hardcoded value.")
        else:
            logging.warning("Drupal.settings JSON not found. Attempting regex fallback for AJAX parameters.")
            ...
            if match_libs:
                ...
                logging.warning("Could not extract ajax_page_state[libraries] via regex. AJAX calls might fail.")
            ...
            if dom_id_match:
                ...
                logging.warning("Could not extract view_dom_id via regex. Using hardcoded value.")
    ...
    while page_num <= max_pagination_attempts:
        ...
        if ajax_libraries_param:
            ...
            logging.warning(f"ajax_page_state[libraries] was empty. Proceeding without it for page {page_num}, but requests may fail.")


        logging.info(f"Fetching page {page_num} via AJAX from {AJAX_ENDPOINT} with params: {request_params}")
        ...
        try:
            ...
            for command in response_json:
                ...
                if command.get("command") == "insert" and "data" in command:
                    ...
                    if command.get("selector") == ".js-view-dom-id-" + view_dom_id:
                    ...
                    elif command.get("selector") == ".block-views-blocktopics-topic-block .view-content": 
                         new_cards_html = command["data"]
                         logging.debug(f"Found 'insert' command with .block-views-blocktopics-topic-block selector for page {page_num}.")
                         break
                    elif command.get("selector") == "#block-knowledge-theme-content-block-2": 
                        new_cards_html = command["data"]
                        logging.debug(f"Found 'insert' command with #block-knowledge-theme-content-block-2 selector for page {page_num}.")
                        break
                    elif command.get("selector") == ".views-element-container": 
                        new_cards_html = command["data"]
                        logging.debug(f"Found 'insert' command with .views-element-container selector for page {page_num}.")
                        break
    ...
    print(f"✅ Done. {added_count[0]} new articles added. {skipped_duplicates_count[0]} duplicates skipped. See insead_scrape.log for details.")
